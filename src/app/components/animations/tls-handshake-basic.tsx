...
import { Monitor, Server, RefreshCw, Pause, Play, Shield, KeyRound, Lock, FileCheck, ArrowRight, CheckCircle2, Send } from "lucide-react";
import { useGlobalAnimationSpeed, AnimationSpeedControl } from "./animation-speed-store";

export function TLSHandshakeBasicAnimation() {
  ...
  return (
    <div className="w-full max-w-3xl mx-auto p-6 bg-gray-50 rounded-xl border border-gray-200 my-6 shadow-sm">
      <div className="flex justify-between items-start mb-8">
        <h3 className="text-lg font-bold text-gray-800 m-0 pt-1">TLS Handshake — Simplified</h3>
          ...
          <div className="flex flex-col items-center gap-1 min-h-[4rem]">
            ...
            {step >= 4 && (
              <div className={`flex items-center gap-1 text-[10px] font-medium transition-all duration-500 ${step === 4 ? "text-indigo-600 scale-110" : "text-indigo-600"}`}>
                <KeyRound size={10} /> ECDH Key Share
              </div>
            )}
            {step >= 5 && (
        ...
        <div className="flex-1 relative mx-4 mt-8">
          ...
          <style>
            {`
              ...
              .msg-slide-right {
                animation: msgSlideRight ${5 / speed}s ease-in-out forwards;
              }
              .msg-slide-left {
                animation: msgSlideLeft ${5 / speed}s ease-in-out forwards;
              }
            `}
          </style>
          ...
          <div className="relative h-32">
            {/* Step 0: Client Hello */}
            {step === 0 && (
              <div className="absolute top-4 msg-slide-right">
                <div className="bg-sky-50 px-3 py-1.5 rounded-lg shadow-md border border-sky-200 flex items-center gap-1.5 whitespace-nowrap">
                  ...
                  <span className="text-xs font-semibold text-sky-700">Client Hello</span>
                </div>
              </div>
            )}
            ...
            {/* Step 1: Server Hello */}
            {step === 1 && (
              <div className="absolute top-4 msg-slide-left">
                <div className="bg-violet-50 px-3 py-1.5 rounded-lg shadow-md border border-violet-200 flex items-center gap-1.5 whitespace-nowrap">
                  ...
                  <span className="text-xs font-semibold text-violet-700">Server Hello</span>
                </div>
              </div>
            )}
            ...
            {step === 2 && (
              <div className="absolute top-4 msg-slide-left">
                <div className="bg-amber-50 px-3 py-1.5 rounded-lg shadow-md border border-amber-200 flex items-center gap-1.5 whitespace-nowrap">
                  ...
                  <span className="text-xs font-semibold text-amber-700">Certificate</span>
                </div>
              </div>
            )}

            {/* Step 3: Verification happening on client */}
            {step === 3 && (
            ...
            {step === 4 && (
              <>
                <div className="absolute top-2 msg-slide-right">
                  <div className="bg-indigo-50 px-2 py-1 rounded-lg shadow-md border border-indigo-200 flex items-center gap-1 whitespace-nowrap">
                    <KeyRound size={11} className="text-indigo-500" />
                    <span className="text-[11px] font-semibold text-indigo-700">ECDH share</span>
                  </div>
                </div>
                <div className="absolute top-12 msg-slide-left">
                  <div className="bg-indigo-50 px-2 py-1 rounded-lg shadow-md border border-indigo-200 flex items-center gap-1 whitespace-nowrap">
                    <KeyRound size={11} className="text-indigo-500" />
                    <span className="text-[11px] font-semibold text-indigo-700">ECDH share</span>
                  </div>
                </div>
                <div className="absolute top-24 left-1/2 -translate-x-1/2 bg-amber-50 px-3 py-1 rounded border border-amber-200 animate-pulse">
                  <span className="text-[10px] font-bold text-amber-700">Shared Secret</span>
                </div>
              </>
            )}

            {/* Step 5: Server signs, client verifies */}
            {step === 5 && (
              <div className="absolute top-4 msg-slide-left">
                <div className="bg-violet-50 px-3 py-1.5 rounded-lg shadow-md border border-violet-200 flex items-center gap-1.5 whitespace-nowrap">
                  <Shield size={12} className="text-violet-500" />
                  <span className="text-xs font-semibold text-violet-700">Signature (ECDSA)</span>
                </div>
              </div>
            )}
      ...
      {/* Step indicators - algorithm badges */}
      <div className="mt-4 flex justify-center gap-2 flex-wrap">
        {[
          { label: "Key Exchange", active: step >= 4, color: "indigo" },
          { label: "Digital Signature", active: step >= 5, color: "violet" },
          { label: "Certificates", active: step >= 2, color: "amber" },
          { label: "Symmetric encryption", active: step >= 6, color: "emerald" },
        ].map((alg) => (
          <span
            key={alg.label}
      ...
      <div className="mt-4 bg-white p-3 rounded-lg border text-sm text-gray-600 text-center min-h-[3.5rem] flex items-center justify-center shadow-sm">
        ...
        {step === 6 && <p><strong>Step 7:</strong> Both derive <strong>AES symmetric keys</strong> from the shared secret. All further data is encrypted. <strong>Secure channel established!</strong></p>}
      </div>
